...
Hoare triples rely on \emph{over-approximation} of program behaviors. This means that the set $[[c]]P$ includes all possible outcomes starting from states satisfying $P$, possibly including states that are not actually reachable, ensuring \emph{no false negatives} in verifying correctness.
...
\minisec{(Structural and Control Rules)}
...
\minisec{(Remaining Inference Rules)}
...
When composing two computations \(f\) and \(g\), if \(f\) returns an \ml{Ok} result then \(g\) is always executed; if \(f\) gets stuck the state is returned immediately, without executing \(g\); errors are turned into stuck states in SL+, while in ISL+ execution continues but \ml{Ok} results of \(g\) are turned into errors.
