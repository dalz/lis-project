...
let assert_eq (name : string) (got : prop) (expected : prop) =
  if got = expected then
    ...
    Printf.printf "FAIL: %s\n  got: %s\n  expected: %s\n"
      name (show_prop got) (show_prop expected)

let () =
  assert_eq "And(True, True)"
    (simplify_prop (And (a_true, a_true)))
    a_true;

  assert_eq "And(True, False)"
    (simplify_prop (And (a_true, a_false)))
    a_false;
  
  assert_eq "And(False, False)"
    (simplify_prop (And (a_false, a_false)))
    a_false;

  assert_eq "And(False, True)"
    (simplify_prop (And (a_false, a_true)))
    a_false;
  ...
  assert_eq "Or(True, True)"
    (simplify_prop (Or (a_true, a_true)))
    a_true;

  assert_eq "Or(True, False)"
    (simplify_prop (Or (a_true, a_false)))
    a_true;

  assert_eq "Or(False, True)"
    (simplify_prop (Or (a_false, a_true)))
    a_true;

  assert_eq "Or(False, False)"
    (simplify_prop (Or (a_false, a_false)))
    a_false;
  ...
  assert_eq "Cmp(Le, 2, 1)"
    ...
    assert_eq "Sep(True, empty)"
    (simplify_prop (Sep (a_true, Atom Emp)))
    a_true;
  ...
  assert_eq "Sep(empty, True)"
    (simplify_prop (Sep (Atom Emp, a_true)))
    a_true;
